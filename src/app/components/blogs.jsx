...
export const Blogs = async () => {
  ...
  const categoriesResponse = await fetch("https://next-mock-api.vercel.app/api/posts/categories");
      ...
      <div className="flex justify-between mb-8">
        <div className="flex gap-5">
          ...
          {categoriesData.map((category, index) => (
            <Link key={index} href={"/category/" + category} className="text-xs font-bold text-[#495057]">
              {category}
            </Link>
          ))}
        </div>
